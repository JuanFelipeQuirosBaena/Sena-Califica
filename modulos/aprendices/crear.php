...
if($_POST){
    ...
    elseif(!filter_var($email,FILTER_VALIDATE_EMAIL)){ $error='Email inválido.'; }
        ...
        $stmt = $db->prepare('SELECT id FROM aprendices WHERE email=? OR documento=?'); $stmt->execute([$email,$documento]);
        if($stmt->rowCount()>0) $error='Email o documento ya registrados.';
            ...
            if(!$error){
                $stmt = $db->prepare('INSERT INTO aprendices (documento,tipo_documento,nombre,apellido,email,telefono,direccion,ficha_id) VALUES (?,?,?,?,?,?,?,?)');
                if($stmt->execute([$documento,$tipo,$nombre,$apellido,$email,$telefono,$direccion,$ficha_id])) $success='Aprendiz creado.'; else $error='Error al crear aprendiz.';
            }
        }
    }
}
?>
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Crear Aprendiz</title>
...
<div class="d-flex justify-content-between align-items-center pt-3 pb-2 mb-3 border-bottom"><h1 class="h2">Crear Aprendiz</h1><a href="index.php" class="btn btn-secondary">Volver</a></div>
