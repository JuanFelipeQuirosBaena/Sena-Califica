...
if($_POST){
    ...
    if(!$numero || !$programa || !$jornada || !$fecha_inicio || !$fecha_fin){ $error='Complete todos los campos.'; }
        ...
        $stmt=$db->prepare('SELECT id FROM fichas WHERE numero_ficha=?'); $stmt->execute([$numero]);
        if($stmt->rowCount()>0) $error='Número de ficha ya existe.';
        else{
            $stmt=$db->prepare('INSERT INTO fichas (numero_ficha,programa_formacion,fecha_inicio,fecha_fin,jornada) VALUES (?,?,?,?,?)');
            if($stmt->execute([$numero,$programa,$fecha_inicio,$fecha_fin,$jornada])) $success='Ficha creada.'; else $error='Error al crear ficha.';
        }
    }
}
?>
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Crear Ficha</title>
...
<div class="d-flex justify-content-between align-items-center pt-3 pb-2 mb-3 border-bottom"><h1 class="h2">Crear Ficha</h1><a href="index.php" class="btn btn-secondary">Volver</a></div>
